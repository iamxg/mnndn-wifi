...
class wifiExperiment:
    ...
    def createFib(self, convergenceTime = None):
        ...
        for sta in self.nodeSet:
            ...
            if "wlan" in wlanFaceList:
                ...
                percentLoss = "0.3% 25%" # Scope value of loss rate sepecified
                staWifiInt = str(sta) + "-wlan0"
                sta.cmd("tc qdisc add dev %s root netem loss %s" % (staWifiInt, percentLoss))
        ...
        if self.isVndn or self.isSumoVndn:
            for carsta in self.carSTASet:
                ...
                if "wlan" in wlanFaceList:
                    self.wlanFaceId=re.search("faceid=(.*?) ", str(wlanFaceList)).group(1)
                    percentLoss = "0.3% 25%" # Scope value of loss rate sepecified
                    carstaWifiInt = str(carsta) + "-wlan0"
                    sta.cmd("tc qdisc add dev %s root netem loss %s" % (carstaWifiInt, percentLoss))
                    carsta.cmd("nfdc route add /ndnwifi/" + " " + self.wlanFaceId + "&")
                else:
                    didNetworkCoverge = False
            ...
            if "wlan" in wlanFaceList:
                ...
                for other in self.nodeSet:
                    if sta.name != other.name:
                        sta.cmd("nfdc route add /ndn/" + str(other) + "-site/" + str(other) +" " + self.wlanFaceId + " &")
            else:
               didNetworkCoverge = False
    ...
    def extractData(self, nodeSet, consumerSet, dataFilePath, isCarSTA):
        ...
        for sta in nodeSet:
            ...
            firstInData = True
            numOutInterest = 0 # The number of interest pactets that sent before received the first data packet
    ...
    def statisticResult(self, nodeSet, consumerSet, dataFilePath, isCarSTA):
        ...
        numOutInterest = 0
        numOutData = 0
        conNumOutInt = 0 # Get the number of interest packet that consumers sent before it received the first data packet. 
        intPacLossRate = 0
        dataPacLossRate = 0
        pacLossRate = 0
        aveDelay = 0
        ...
        for line in input_file:
            ...
            while i < len(line):
                if not (line[i] == " "):
                    ...
                    lineString = ""
                i = i+1
            # Only consider the delay of consumer.
            if (lineField[8].strip() != "9999999999.999999") and (lineField[9].strip() != "9999999999.999999") and int(lineField[1].strip()) != 0:
            ...
            # Only consider the number of interest packet that the consumer sent.
        ...
        if numOutInterest != 0:
            intPacLoss = abs(numOutInterest-numInInterest)
            intPacLossRate = float(intPacLoss) / float(numOutInterest)
        if numOutData != 0:
            dataPacLoss = abs(numOutData-numInData)
            dataPacLossRate = float(dataPacLoss) / float(numOutData)
        if ((numOutInterest != 0)  or (numOutData !=0 )):
            pacLoss = abs((numOutInterest + numOutData) - (numInInterest + numInData))
            pacLossRate = float(pacLoss) / float(numOutInterest + numOutData)
        statResult = str(self.nConsumers) + " " + str(self.nPings) + " " + str(conNumOutInt/self.nConsumers) + " " + str(aveDelay/self.nConsumers) \
