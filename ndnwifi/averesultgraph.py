...
# Read data file and convert to two dimession List.
...
def aveStatResult(dataFilePath, dataFileDir, dataFileName, aveCol):
    ...
    while i < len(sortDataList):
        avePLR = float(sortDataList[i][2].strip())
        ...
        while (j < len(sortDataList)) and flag:
            if sortDataList[j][aveCol] == tmp:
                n = n + 1
                avePLR = avePLR + float(sortDataList[j][2].strip())
                aveISR = aveISR + float(sortDataList[j][3].strip())
                aveAVD = aveAVD + float(sortDataList[j][4].strip())
                j = j+1
            else:
                flag = False
        i = j
    ...
    return conNodes, pacLossRate, intSatRate, aveDelay
...
def drawStatGraph(dataFilePath, dataFileDir, aveCol):
    ...
    for dataFileName in dataFileList:
        conNodes,pacLossRate, intSatRate, aveDelay = aveStatResult(dataFilePath, dataFileDir, dataFileName, aveCol)
        ...
        plt1.plot(conNodes, pacLossRate, color=cmap(i), linestyle='solid', label = labelChar,marker='s', markerfacecolor=cmap(i), markersize=10)
