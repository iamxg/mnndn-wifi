...
def aveStatResult(dataFilePath, dataFileDir, dataFileName, aveCol):
    ...
    while i < len(sortDataList):
        aveIDR = float(sortDataList[i][2].strip())
        aveISR = float(sortDataList[i][3].strip())
        aveAVD = float(sortDataList[i][4].strip())
        aveIntPLR = float(sortDataList[i][5].strip())
        aveDataPLR = float(sortDataList[i][6].strip())
        avePLR = float(sortDataList[i][7].strip())
        ...
        intDelRate.append(aveIDR/n)
        intSatRate.append(aveISR/n)
        aveDelay.append(aveAVD/n)
        intPacLossRate.append(aveIntPLR/n)
        dataPacLossRate.append(aveDataPLR/n)
        pacLossRate.append(avePLR/n)

    return conNodes, intDelRate, intSatRate, aveDelay, intPacLossRate, dataPacLossRate, pacLossRate
...
def drawStatGraph(dataFilePath, dataFileDir, aveCol):
    ...
    for dataFileName in dataFileList:
        conNodes, intDelRate, intSatRate, aveDelay, intPacLossRate, dataPacLossRate, pacLossRate = \
        ...
        plt1.plot(conNodes, intDelRate, color=cmap(i), linestyle='solid', label = labelChar,marker='s', markerfacecolor=cmap(i), markersize=10)
        plt2.plot(conNodes, intSatRate, color=cmap(i), linestyle='solid', label = labelChar,marker='s', markerfacecolor=cmap(i), markersize=10)
        plt3.plot(conNodes, aveDelay, color=cmap(i), linestyle='solid',label=labelChar, marker='s', markerfacecolor=cmap(i), markersize=10)
        plt4.plot(conNodes, intPacLossRate, color=cmap(i), linestyle='solid', label = labelChar,marker='s', markerfacecolor=cmap(i), markersize=10)
...
#    plt3.plot(nodes, aveDelay, color='red', linestyle='dashed')
    plt1.set_title('Interest Packet Successfully Delivery Rate')
    plt2.set_title('Interest Satisfaction Rate')
    plt3.set_title('Average Delay')
    plt4.set_title('Packet Loss Rate')
    #plt1.xlabel('nodes')
    #plt1.ylabel('ISR')
