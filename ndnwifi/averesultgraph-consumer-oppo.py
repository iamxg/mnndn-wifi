...
def aveStatResult(dataFilePath, dataFileDir, dataFileName, aveCol):
    ...
    while i < len(sortDataList):
        ...
        avePLR.append(PLR/n)
        print conNodes, "|||", aveConNumOutInt, "||||", conNumOutInt,"|||", n
...
def drawStatGraph(dataFilePath, dataFileDir, aveCol):
    ...
    # n is times (here n=5 because the number of consumers is {1,3,5,7,9} ) for opening these files.
    ...
    while n < 5:
        ...
        conNum = []
        aveCNOI = [] #Sent the number of interest packets before a consumer receives the first data packet.
        aveDLY = [] # delay
