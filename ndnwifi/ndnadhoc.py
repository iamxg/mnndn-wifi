...
def build_adhocnet(adhocTopo, isManet, isAdhoc, ssid, channel, mode, wmediumd, interference,workdir,
                   cluster, placement, servers, tunnelType, ctime, nPings, nConsumers, pctTraffic, experimentName, nlsrSecurity):
    """Create an ad hoc network from a topology object
       ...
       nConsumers: number of the consumers 
    """
    ...
    adhocnet.plotGraph(max_x=200, max_y=200, max_z=0)

    if isManet:
        #adhocnet.seed(random.randint(0, 100))
        adhocnet.seed(50)
        # this code line must be put here
        adhocnet.startMobility(time=0,  model='RandomWayPoint', max_x=200, max_y=200, max_z=0, min_v=0.5, max_v=0.8)
    adhocnet.build()
